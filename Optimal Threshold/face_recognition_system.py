...
class FaceRecognitionSystem:
    def __init__(self, detection_model_path, recognition_model_path, known_faces_path, debug_mode=False):
        ...
        self.strict_threshold = 0.91  # High confidence threshold
        self.min_matching_embeddings = 5  # Must match at least 3 training images
        
        # Welcome display settings
        self.welcome_duration = 15  # Duration to display welcome message in seconds (1 minute)
        self.in_welcome_mode = False  # Track if currently in welcome mode
        self.welcome_start_time = None  # When welcome mode started
        self.last_detected_box = None  # Last detected face position for Mr. David
    ...
    def draw_welcome_message(self, image, box, remaining_time):
        """
        Draw prominent welcome message during welcome mode
        
        Args:
            image: Image to draw on
            box: Bounding box coordinates (x1, y1, x2, y2)
            remaining_time: Seconds remaining in welcome mode
        """
        x1, y1, x2, y2 = box
        ...
        timer_text = f"Welcome active: {int(remaining_time)}s remaining"
        ...
        corner_length = 20
        # Top-left corner
        cv2.line(image, (x1, y1), (x1 + corner_length, y1), color, 4)
        cv2.line(image, (x1, y1), (x1, y1 + corner_length), color, 4)
        
        # Top-right corner
        cv2.line(image, (x2, y1), (x2 - corner_length, y1), color, 4)
        cv2.line(image, (x2, y1), (x2, y1 + corner_length), color, 4)
        
        # Bottom-left corner
        cv2.line(image, (x1, y2), (x1 + corner_length, y2), color, 4)
        cv2.line(image, (x1, y2), (x1, y2 - corner_length), color, 4)
        
        # Bottom-right corner
        cv2.line(image, (x2, y2), (x2 - corner_length, y2), color, 4)
        cv2.line(image, (x2, y2), (x2, y2 - corner_length), color, 4)
        
        # Add a centered large welcome message at the top of the frame
        h, w = image.shape[:2]
        big_text = "WELCOME!"
        ...
        cv2.rectangle(image, (big_text_x - 20, 60), 
                     (big_text_x + big_text_size[0] + 20, 120), (255, 0, 0), -1)
    ...
    def run(self):
        ...
        With 1-minute welcome display when Mr. David is recognized
        """
        ...
        print(f"Welcome display duration: {self.welcome_duration} seconds")
        print("Press 'q' to quit")
        print("Press 's' to save current frame")
        print("Press 'r' to reset welcome mode")
        if self.debug_mode:
        ...
        while True:
            ...
            # Check if we're in welcome mode
            if self.in_welcome_mode:
                ...
                if remaining_time > 0:
                    ...
                    if self.last_detected_box is not None:
                        face_box = self.last_detected_box
                    else:
                        # Try to detect Mr. David's current position
                        faces = self.detect_faces(frame)
                        if len(faces) > 0:
                            face_box = faces[0]  # Use first detected face
                            self.last_detected_box = face_box
                        else:
                    ...
                    self.draw_welcome_message(display_frame, face_box, remaining_time)
                    ...
                    print(f"\n✅ Welcome period ended. Resuming normal detection...")
                ...
                # Track if Mr. David was recognized in this frame
                mrdavid_recognized = False
            ...
            if key == ord('q'):
                ...
                # Reset welcome mode (manual override)
                if self.in_welcome_mode:
